...
-- DBTITLE 1,SQL Like Syntax
CREATE OR REFRESH STREAMING TABLE customers_raw
AS SELECT *
  , current_timestamp() as load_time
FROM cloud_files('/Volumes/masterdb_dev/demo_db/landing_zone/customers/'
                  ,"csv"
...
AS SELECT *
  , current_timestamp() as load_time
FROM cloud_files("/Volumes/masterdb_dev/demo_db/landing_zone/invoices/"
                  ,"csv"
                  , map("cloudFiles.inferColumnTypes", "true")
                )
