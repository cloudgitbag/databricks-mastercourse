...
#         df_micro_batch._jdf.sparkSession().sql(self.merge_query) # runs the merge query
...
class Upserter:
    ...
    def upsert(self, df_micro_batch, batch_id: int):
        # Register the batch as a temp view in the same session that created this df
        df_micro_batch.createOrReplaceTempView(self.temp_view_name)

        # ✅ Use the public, Connect-compatible session handle
        spark = df_micro_batch.sparkSession

        # Execute your MERGE (SQL text or DataFrame API)
...
class Silver():
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_delta = (spark.readStream
                         ...
                         .dropDuplicates(["user_id", "device_id"]) #UK/PK
                   )
        ...
        #https://spark.apache.org/docs/latest/api/python/reference/pyspark.ss/api/pyspark.sql.streaming.DataStreamWriter.foreachBatch.html

        stream_writer = (df_delta.writeStream
                                 .foreachBatch(data_upserter.upsert) #UDF df_delta - auto pass
                                 .outputMode("update")
