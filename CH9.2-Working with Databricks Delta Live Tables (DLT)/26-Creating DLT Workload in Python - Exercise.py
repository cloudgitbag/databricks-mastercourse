...
@dlt.table(name="customers_raw2")
def get_customers_raw():
  return (spark.readStream
               .format("cloudFiles")
               .option("cloudFiles.format", "csv")
               .option("cloudFiles.inferColumnTypes", "true")
               .load("/Volumes/masterdb_dev/demo_db/landing_zone/customers2/")
               .withColumn("load_time", current_timestamp())
        )

# COMMAND ----------

@dlt.table(name="invoices_raw2")
def get_invoices_raw():
  return (spark.readStream
               .format("cloudFiles")
               .option("cloudFiles.format", "csv")
               .option("cloudFiles.inferColumnTypes", "true")
               .load("/Volumes/masterdb_dev/demo_db/landing_zone/invoices2/")
               .withColumn("load_time", current_timestamp())
        )
...
dlt.create_streaming_table("customers2")

dlt.apply_changes(
  target = "customers2",
  source = "customers2_cleaned",
  keys = ["customer_id"],
...
dlt.create_streaming_table("invoices2", partition_cols = ["invoice_year", "country"])

dlt.apply_changes(
  target = "invoices2",
  source = "invoices2_cleaned",
...
@dlt.table(name="daily_sales_uk_2021")
def compute_daily_sales_uk_2021():
  return (spark.read #not readstream
               .format("delta")
               .table("live.invoices2")
               .where("invoice_year = 2021 AND country = 'United Kingdom'")
