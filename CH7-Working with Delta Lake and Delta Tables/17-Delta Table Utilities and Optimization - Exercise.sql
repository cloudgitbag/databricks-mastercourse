...
-- MAGIC
-- MAGIC         spark.sql("DROP TABLE IF EXISTS masterdb_dev.demo_db.fire_calls_tbl")
-- MAGIC                   
-- MAGIC         raw_df = (spark.read
-- MAGIC                 .format("csv")
-- MAGIC                 .option("header", "true")
-- MAGIC                 .option("inferSchema","true")
-- MAGIC                 .load("/Volumes/dev/spark_db/datasets/spark_programming/data/sf-fire-calls.csv")
-- MAGIC         )
-- MAGIC
...
-- MAGIC                 .saveAsTable("masterdb_dev.demo_db.fire_calls_tbl")
-- MAGIC         )
-- MAGIC setup()        
...
DESCRIBE EXTENDED masterdb_dev.demo_db.fire_calls_tbl
...
-- SELECT * FROM masterdb_dev.demo_db.fire_calls_tbl

Delete FROM masterdb_dev.demo_db.fire_calls_tbl 
WHERE CallDate ='2002-01-24'

-- COMMAND ----------

UPDATE masterdb_dev.demo_db.fire_calls_tbl 
SET Delay =int(Delay)
...
DESCRIBE HISTORY masterdb_dev.demo_db.fire_calls_tbl 
...
SELECT * FROM masterdb_dev.demo_db.fire_calls_tbl  VERSION AS OF 0
...
VACUUM masterdb_dev.demo_db.fire_calls_tbl RETAIN 168 HOURS DRY RUN
...
ALTER TABLE masterdb_dev.demo_db.fire_calls_tbl SET TBLPROPERTIES ('delta.columnMapping.mode' = 'name')
...
ALTER TABLE masterdb_dev.demo_db.fire_calls_tbl DROP columns(SupervisorDistrict, FirePreventionDistrict)
...
REORG TABLE masterdb_dev.demo_db.fire_calls_tbl APPLY(PURGE)
...
OPTIMIZE masterdb_dev.demo_db.fire_calls_tbl ZORDER BY (Year, CallDate)
