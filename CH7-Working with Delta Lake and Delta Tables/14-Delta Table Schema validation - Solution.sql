...
CREATE OR REPLACE TABLE masterdb_dev.demo_db.people_tbl(
  id INT,
  firstName STRING,
  lastName STRING
) USING DELTA;
...
INSERT OVERWRITE masterdb_dev.demo_db.people_tbl
SELECT id, fname, lname, dob
FROM json.`/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json`
...
SELECT id, fname, lname FROM json.`/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json`

-- COMMAND ----------

MERGE INTO masterdb_dev.demo_db.people_tbl tgt
USING (SELECT id, fname, lname FROM json.`/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json`) src
ON tgt.id = src.id
...
SELECT id, fname firstName, lname lastName, dob FROM json.`/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json`

-- COMMAND ----------

MERGE INTO masterdb_dev.demo_db.people_tbl tgt
USING (SELECT id, fname firstName, lname lastName, dob FROM json.`/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json`) src
ON tgt.id = src.id
WHEN NOT MATCHED THEN
    INSERT *
...
-- MAGIC people_df =  spark.read.format("json").schema(people_schema).load("/Volumes/masterdb_dev/demo_db/files/dataset_ch7/people.json")
-- MAGIC
-- MAGIC people_df.write.format("delta").mode("append").saveAsTable("masterdb_dev.demo_db.people_tbl")
